 ...
 */
async function generateBarrelContent(fullDir: string, dir: string): Promise<string | null> {
  ...
  for (const file of sourceFiles.sort((a, b) => a.localeCompare(b))) {
    ...
    if (namedExports.length > 0) {
      ...
      exportLines.push(`export { default as ${pascalName} } from './${moduleName}.js';`);
    }
  }
  ...
  async fix(context: ScanContext, violations: ScanViolation[]): Promise<FixResult> {
    ...
    for (const violation of violations) {
      ...
      try {
        ...
        const barrelContent = await generateBarrelContent(fullDir, dir);
